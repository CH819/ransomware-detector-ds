...
RECOVERY_MANAGER_URL = os.environ.get(
    "RECOVERY_MANAGER_URL", "http://load_balancer:6000/recover"
)
CLIENT_BASE_URL = os.environ.get("CLIENT_BASE_URL", "http://load_balancer:7000")
...
class Gateway:
    ...
    def get_nodes_infection_timestamps(self) -> dict[str, str]:
        """Get all node infection timestamps from Redis."""
        return self.redis.hgetall("gateway:infection_timestamp")
    ...
    def reset_node(self, node_id: str, reason: str = "recovery_complete"):
        ...
        self.redis.hdel("gateway:infection_timestamp", node_id)
    ...
    def handle_detector_result(self, result: dict):
        ...
        if decision in ["ISOLATE", "BACKUP"]:
            # Set infection timestamp if not already set
            self.redis.hsetnx(
    ...
    def recover_node(self, node_id: str):
        ...
        # Get infection timestamp to help recovery manager pick the right snapshot
        infection_timestamp = self.redis.hget("gateway:infection_timestamp", node_id)
        ...
        try:
            recovery_response = requests.post(
                ...
                json={
                    "node_id": node_id,
                    "infection_timestamp": infection_timestamp,
                },
                timeout=10,
            )
            ...
            if recovery_response.status_code != 200:
                self.logger.error(
                    f"Failed to get snapshot for node {node_id}: {recovery_response.status_code} {recovery_response.text}"
                )
                self.set_node_status(
                    node_id, NodeStatus.ISOLATED, "failed_to_get_snapshot"
                )
            ...
            if client_response.status_code != 200:
                self.set_node_status(
                    node_id, NodeStatus.ISOLATED, "client_restore_failed"
                )
